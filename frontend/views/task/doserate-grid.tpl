<div>

    {* Replay map*}
    <div id="map" style="width: 100%; height: 500px; margin-bottom: 50px">

    </div>

    <div class="box">
        <table>
            <tr>
                <td style="background-color: rgba(0, 0, 225, 0.5)">&nbsp;</td>
                <td style="background-color: rgba(0, 255, 0, 0.5)">&nbsp;</td>
                <td style="background-color: rgba(255, 255, 0, 0.5)">&nbsp;</td>
                <td style="background-color: rgba(255, 80, 0, 0.5)">&nbsp;</td>
                <td style="background-color: rgba(255, 0, 0, 0.5)">&nbsp;</td>
            </tr>
            <tr>
                <td width="80px"><100</td>
                <td width="80px">100-200</td>
                <td width="80px">200-300</td>
                <td width="80px">300-400</td>
                <td width="80px">>400</td>

            </tr>
            <tr>

            </tr>
        </table>
    </div>

    <div class="box">
        <h4 class="col-md-4 control-label" id="timeRange">

        </h4>
    </div>

    <div class="box">
        <div class="col-md-4">
            <div class="slider-controls slider-value-top">
            </div>
            <div id="slider-range" class="slider slider-red">
            </div>
        </div>
    </div>

    <div class="box">
        <a class="btn btn-grey">返回</a>
        <a class="btn btn-info start" >
            <i class="fa fa-play"></i>&nbsp;显示
        </a>

    </div>
</div>