<div class="dataTables_paginate paging_bs_full">
    <ul id='pagination' class="pagination" form-id="#{$form}" current="{$pager.current}">
        <li {if 1==$pager.current}class="disabled"{/if}><a href="#" tabindex="0" data="prev" class="paginate_button previous">上一页</a></li>
        {foreach from=$pager.items item=p}
            {if $p.type == 'pager'}
                <li {if $p.active}class="active"{/if}><a href="#" tabindex="0" data="{$p.title}">{$p.title}</a></li>
            {else}
        ...
        <li {if $pager.count==$pager.current}class="disabled"{/if}><a href="#" tabindex="0" data="next" class="paginate_button next">下一页</a></li>
    </ul>
</div>
<script>
